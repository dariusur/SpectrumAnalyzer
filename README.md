...
4. The external hardware currently is the weakest part of this project. The op-amps are biased at ${Vcc \over 2} = 2.5 V, but the maximum output voltage that can be provided is something like 3.78 V (this varies from one op-amp to another). This causes underuse of the possible voltage range and prevents the signal from reaching 1 in amplitude on the graph. Currently, the maximum it can reach is 0.76, after that, clipping occurs and harmonics arise. I'm not sure if it's a good idea to bias the op-amp at 1.89 V because that would mean the greatest expected output from an op-amp is 3.78 V, but who is to say that it will never swing above? After all, the datasheet only lists the maximum deviation from the V+ rail. In such an event, the analog pin of the FPGA might see a voltage that is above the allowable range. To solve this, maybe a look into rail-to-rail op-amps would be worthy, or some kind of a protection circuit that would clip output voltages above 3.3 V would also be nice.
